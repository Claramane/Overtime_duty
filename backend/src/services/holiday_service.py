...
class HolidayService:
    ...
    def _load_holidays(self):
        ...
        try:
            # 嘗試相對於目前工作目錄載入 (假設腳本從專案根目錄執行)
            if not os.path.exists(self.holiday_file):
                # 如果在根目錄找不到，嘗試相對於服務檔案的位置載入
                script_dir = os.path.dirname(__file__)
                potential_path = os.path.join(script_dir, '..', '..', self.holiday_file) # 退兩層到根目錄再進 data/
                logger.debug(f"Holiday file not found at {self.holiday_file}, trying relative path: {potential_path}")
                if os.path.exists(potential_path):
                    self.holiday_file = potential_path
                else:
                     logger.error(f"錯誤：找不到假日檔案 {self.holiday_file} 或 {potential_path}")
